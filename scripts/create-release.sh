...
if [[ -n "$PREVIOUS_TAG" ]]; then
    awk \
        ...
        # Début de la précédente version définitive, on arrête AVANT cette ligne.
        $0 ~ "^## \\[" previous "\\]" {
            if (in_release) {
                exit
            }
    ...
    # Première version définitive, on prend tout le CHANGELOG à partir de la section courante.
    awk \
        -v current="$TAG" '
        BEGIN {
            in_release = 0
